...
class App{
	...
	protected $actions = ["user","page","contact","server","pricing","upgrade","cash_out","analytic","profile","ipn", "q", "jShortener", "webhook","api", "scriptjs", "r","blog", "report", "u","affiliate","referral_link"];
	/**
	 * User Variables
	 * @since 4.0
	 **/
	...
	protected function user(){
		...
		if($this->logged()){
			...
			$action = array("edit","delete","archive","expired","bundles","splash","settings","logout","verify","search","server","export","overlay","pixels", "tools", "cancel","terminate","membership","domain","teams", "builder", "cash_out",'affiliate','affiliateDashboard');
	...
	*/
	...
	protected function user_menu($option=array()){
		$menu='<ul class="nav nav-sidebar">';
			...
			if($this->user->type === 'affiliate'){
                ...
                $menu.='<li><a href="'.Main::href("user/affiliateDashboard").'" class="active"><span class="glyphicon glyphicon-link"></span> '.e('Affilate Dashboard').'</a></li>';
	...
	protected function server(){
		...
		// Swtich requests
		$system=array("unlock","lock","bundle","edit","archive","unarchive","activities","bundle_urls","url_bundle_add","bundle_create","bundle_edit", "cancel","refreshlinks", "bulk_bundle", "bulk_bundle_add", "validatecoupon", "cash_out");	
  ...
  /**
    ...
    protected function affiliate()
    {
        ...
        if(isset($_POST["token"])){
             ...
             // Validate Token
             if(!Main::validate_csrf_token($_POST["token"])){
                 return Main::redirect("affiliate",array("danger",e("Something went wrong, please try again.")));
             }
             if(empty($_POST["name"]) || !Main::email($_POST["email"])){
                 return Main::redirect("affiliate",array("danger",e("Please fill everything")."!"));
             }
             // Check Captcha
             if($this->config["captcha"]){
                 $captcha=Main::check_captcha($_POST);
                 if($captcha!='ok'){
                     return Main::redirect("affiliate",array("danger",$captcha));
                 }
             }
            $affiliate_data = [
                ":name"  => Main::clean($_POST["name"],3,TRUE),
                ":email"  => Main::clean($_POST["email"],3,TRUE),
                ":paypal_account"  => Main::clean($_POST["paypal_account"],3,TRUE),
                ":website_brand"  => Main::clean($_POST["website_brand"],3,TRUE),
                ":affiliate_link"  => Main::clean($_POST["affiliate_link"],3,TRUE),

            ];

            $this->db->insert("affiliates", $affiliate_data);

            return Main::redirect("affiliate",array("success",e("Your request has been sent. We will reply you as soon as possible.")));
        }



        $this->header();
        include($this->t("affliate"));
        $this->footer();
    }


    protected function referral_link()
    {

        $clicks =$this->db->count("affiliates","","clicks");


        $this->db->update("affiliates", ["clicks" =>$clicks+1], ["referral_link" => $_GET['referral_link']]);

        $_SESSION['referral_link'] = $_GET['referral_link'];
