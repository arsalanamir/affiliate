	...
	protected function register(){
		...
		if(isset($_POST["token"])){
			...
			$user_id = $this->db->pdo()->lastInsertId();		
			    ...
			    /*track if user was register using referral link*/

                $register_users =$this->db->count("affiliates","","register_users");

                $this->db->update("affiliates", ["register_users" =>$register_users+1], ["referral_link" => $_SESSION['referral_link']]);

                $aff = $this->db->get('affiliates',["referral_link" => $_SESSION['referral_link']]);
                $affiliate_id=$aff[0]->id;

                // $user=($this->db->get("user",array("email"=>"?"),"",array($_POST["email"])));
                // $user_id=$user[0]->id;

               

                /*bara chotia ho beta, #this->use this is currently logged in user, user comment snese you cn aget insert userin in less then 30 sec and in one lin eo fcode here! bye*/
 $this->db->insert("affiliate_link_users",[':user_id'=>$user_id,':affiliate_id'=>$affiliate_id]);
   ...
   protected function cash_out()
				...
				// 			if($this->user->trial){
					...
					// 			}
								...
								print_r(" jkfhrskhun");
								return Main::redirect(Main::href("user", "", FALSE), array("danger", e("You cannot Cash out in a Trial / Free account.")));
							}
   ...
   */
   ...
   return Main::redirect(Main::href("user/teams","",FALSE),array("danger",e("Something went wrong, please try again.")));
    ...
    protected function affiliateDashboard(){



	    if($this->user->type === 'affiliate'){

        $affilaite = $this->db->get('affiliates',["email"=>$this->user->email]);

        $this->isUser=TRUE;

        Main::set("title",e("Affiliate Dashboard"));
        $this->header();
        include($this->t("affiliate_dashboard"));
        $this->footer();
