...
class Admin{
   ...
   * @since 5.5
   **/
  protected $actions = ["ads","users","urls","payments","pages","settings","themes","languages","help","search","subscription","tools","plans","update","emails","domains", "blog", "coupons", "reports",'affiliate', "phpinfo"];
  /**
   * Config
   * @since 4.0
   **/
  ...
  protected function PayPalAPIWebhook(){
    ...
    public function generateUniqueAffiliateLink(){

        do{

            $referral_link =  Main::strrand();

        }while(count($this->db->get('affiliates',["referral_link"=>$referral_link])) > 0);
    ...
    public function uniqueUserName($name){
        ...
        }while(count($this->db->get('user',["username"=>$username])) > 0);
...
//      $aff = $this->db->get('affiliates',["id"=>$_GET['id']]);
//       do{

//           $email = ($aff[0]->email);
//          $user = strstr($email, '@', true);
//         $use = $user.rand(1,10)
// /*ther eis one thpe of L an dyou are special! L*/

//       }while(count($this->db->get('user',["username"=>$user])) > 0);
...
//   }
    ...
    protected function affiliate(){
        ...
        if($_GET['status']){
            ...
            /*Send Email when status if verified*/

            $this->db->update("affiliates",  ["status" => $_GET['status']] ,["id" => $_GET['id']]);


            /*Send Email who request of affiliate program have ben approved*/

            if($_GET['status'] === 'Approved'){

                /*First step on approval we have to inform user that he has been accepted to apply for affiliate program
                We may also need to send user credentials
                */

                $aff = $this->db->get('affiliates',["id"=>$_GET['id']]);


                if(!$aff[0]->is_email_sent) {


                    // Check email in database no need to create user again!
                    if (!$this->db->get("user", array("email" => "?"), "", array($aff[0]->email))) {
                        ...
                        // Prepare Data
                        $data = array(
                            ":email" => Main::clean($aff[0]->email, 3),
                            /*Setup default password*/
                            ":password" => Main::encode('123456'),
                            ":auth_key" => $auth_key,
                            ":api" => $unique,
                            ":type" => 'affiliate',
                            /*Here need to remove spaces for example M tariq, so it will be come, Mtariq, next is have to maek it lower case so it should look like username, do it! it will now rnow lpc!*/
                            ":username" => $this->uniqueUserName(strtolower(str_replace(' ', '', ($aff[0]->name)))),
                            ":date" => "NOW()",
                        );
                        ...
                        if ($this->db->insert("user", $data)) {
                            ...
                            $refer_link = $this->generateUniqueAffiliateLink();
                            $this->db->update("affiliates", ["referral_link" => $refer_link], ["id" => $_GET['id']]);

                            // Send Email
                            $mail["to"] = Main::clean($aff[0]->email, 3);
                            $mail["subject"] = "[{$this->config["title"]}] Congrats! Registration successfull for Affiliate program! ";
                            ...
                            $mail["message"] = str_replace("Password:", '123456', $mail["message"]);
                            $mail["message"] = str_replace("Referral Link:", $this->config['url'] . '/referral_link?referral_link=' . $refer_link, $mail["message"]);
                            $mail["message"] = str_replace("{user.date}", date("d-m-Y"), $mail["message"]);
                            Main::send($mail);

                            /*Set Flag to true when email got sent!*/
                            $this->db->update("affiliates",  ["is_email_sent" => true] ,["id" => $_GET['id']]);
        ...
        $affiliates = $this->db->get("affiliates");
        $this->header();
        include($this->t("affiliate_data"));
        $this->footer();
